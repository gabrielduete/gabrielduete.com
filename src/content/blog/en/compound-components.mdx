...
There are various patterns, and in this article, we’ll discuss **Compound
...
The select element works as the interface’s state manager, while the option
elements configure how the select component should function.

# Example Using Compound Components

In this example, we’ll create a `Modal`, which is divided into two compound
...
Notice that we’re using `children` to grab the components that will be placed
...
Finally, we can add both components to our Modal component, and it’s ready to
go! 😊
...
![Visual result of the component’s functionality](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/9j8d40ids8i8iifbdp5t.gif)
...
We’ve seen how writing components using the Compound Components pattern can be
...
together articles created by developers in the React universe. It’s worth
checking out!_
